...
class prophet_js {
    ...
    setup_dataframe(df,initialize_scales = false){
        ...
        */
        ...
        /*
        for name, props in this.extra_regressors.items():
    ...
    set_changepoints(){
        ...
        */
        ...
        if(this.n_changepoints > 0){
            let cp_indexes = new nj.matrix(
                nj.linspace(0, hist_size, this.n_changepoints + 1)
                .round()
                .astype(nj.int)
            );
            this.changepoints = (
    ...
    */ 
    ...
    /*Predict seasonality components, holidays, and added regressors.
        ...
        let components = {'col': nj.arange(seasonal_features.shape[1]),
        ...
        let X = seasonal_features.as_matrix();
        let data = {};
        for(component, features in components.groupby('component')){
            cols = features.col.tolist();
            comp_beta = this.params['beta'][:, cols];
            comp_features = X[:, cols];
            comp = (nj.matmul(comp_features, comp_beta.transpose())
                * this.y_scale);  // noqa W503
